...
```powershell
& "C:\Git\cmd\git.exe" remote add origin https://Lemonpine-ai:YOUR_GITHUB_TOKEN@github.com/Lemonpine-ai/VibeCoading.git
```

### Push 실패 시 (인증·네트워크)
```powershell
# remote URL에 토큰 포함 여부 확인 후, 없으면 재등록
& "C:\Git\cmd\git.exe" remote set-url origin https://Lemonpine-ai:YOUR_GITHUB_TOKEN@github.com/Lemonpine-ai/VibeCoading.git
