...
export function CameraLiveViewer({
  ...
  const connectToLiveSession = useCallback(
    async (session: LiveSession) => {
      ...
      try {
        ...
        // 단계별 ICE 상태 로그 자동 부착 (개발자 도구 콘솔에서 확인 가능)
        attachIceStateLogger(pc, 'viewer')

        let hasReportedFailure = false
        function reportFailureToUser() {
          if (hasReportedFailure) return
          hasReportedFailure = true
          setConnectionPhase('error')
          setErrorMessage(buildWebRtcNetworkFailureUserMessage({ turnRelayConfigured }))
          setLiveSession(null)
        ...
        async function applyBroadcasterIceCandidate(rawCandidate: RTCIceCandidateInit) {
        ...
        pc.oniceconnectionstatechange = () => {
          ...
          if (ice === 'connected' || ice === 'completed') setConnectionPhase('connected')
          if (ice === 'failed') reportFailureToUser()
        }

        pc.onconnectionstatechange = () => {
          const state = pc.connectionState
          logWebRtcDebug('viewer', 'peer.connection_state', { connectionState: state })
          if (state === 'connected') setConnectionPhase('connected')
          if (state === 'failed') {
            reportFailureToUser()
            return
          }
        ...
        pc.onicecandidate = ({ candidate }) => {
          ...
          void (async () => {
            const { error: iceInsertError } = await supabase.from('ice_candidates').insert({
        ...
        await new Promise<void>((resolve, reject) => {
          channel.subscribe((status) => {
            ...
            if (status === 'CHANNEL_ERROR') reject(new Error('ICE 실시간 채널을 열 수 없어요.'))
          })
        })
        ...
        for (const row of existingBroadcasterIceRows ?? []) {
          await applyBroadcasterIceCandidate(row.candidate as RTCIceCandidateInit)
        }
        ...
        const { error: answerUpdateError } = await supabase
          ...
          .update({ answer_sdp: encodePlainSdpForDatabaseColumn(committedAnswer.sdp) })
          .eq('id', session.id)
  ...
  // home_id 확보 후: 최초 환경 관리 타임스탬프 조회
  useEffect(() => {
    if (!homeId) return
  ...
  // Realtime: 다른 기기에서 환경 관리 저장 시 즉시 반영
  useEffect(() => {
    if (!homeId) return
  ...
  useEffect(() => {
    ...
    async function loadExistingSession() {
      const { data: sessions } = await supabase
        ...
        .limit(1)
      if (sessions && sessions.length > 0) setLiveSession(sessions[0] as LiveSession)
    }
      ...
      .on(
        ...
        (payload) => {
          const row = payload.new as {
            status?: string; offer_sdp?: string | null; id?: string; cat_id?: string | null
          }
  ...
  if (!homeId) {
    return (
      <div className="flex min-h-[200px] w-full items-center justify-center rounded-3xl bg-[#F1FBF9]">
        <Loader2 className="size-8 animate-spin text-[#4FD1C5]" />
      </div>
    )
  }
    ...
    >
      {isFigmaVariant ? (
        <div className="mb-3 flex items-center justify-between px-0.5">
          <h2 className="text-sm font-semibold tracking-wide text-[var(--color-primary-dark)]">
            LIVE CAM
          </h2>
          ...
          >
            ...
            {connectionPhase === 'error' && (
              <>
                ...
                <button
                  type="button"
                  onClick={() => { setErrorMessage(null); setConnectionPhase('watching_for_broadcast') }}
      ...
      <div className="relative z-50 mt-4 flex flex-col gap-3 rounded-2xl border border-[#4FD1C5]/35 bg-white p-3 shadow-lg">
        ...
        <div className="flex flex-row flex-wrap gap-2">
          ...
          <button
            type="button"
            disabled={careLogPending}
            onClick={() => void recordCareLog('medicine')}
