...
function persistPairedDeviceCredentials(data: {
  device_token: unknown; device_id: unknown; device_name: unknown; home_id: unknown
}) {
...
/**
 * 4자리 코드 입력 → Supabase RPC 검증 → device_token 저장 → 방송 페이지 이동
 */
export function CameraPairClient() {
  ...
  const handleDigitChange = useCallback((index: number, rawValue: string) => {
    ...
    setDigits((prev) => { const next = [...prev]; next[index] = singleDigit; return next })
    if (singleDigit && index < CODE_LENGTH - 1) inputRefs.current[index + 1]?.focus()
  }, [])
  ...
  async function submitPairingCode() {
    ...
    if (pairingCode.length < CODE_LENGTH) { setErrorMessage('4자리 숫자를 모두 입력해 주세요.'); return }

    setPairingPhase('pairing')
    setErrorMessage(null)

    const { data, error } = await supabase.rpc('pair_camera_device', { input_pairing_code: pairingCode.trim() })
    ...
    const { error: ensureErr } = await supabase.rpc('ensure_camera_session_after_pairing', { p_device_token: String(data.device_token) })
    if (ensureErr) console.warn('[pair] ensure_camera_session_after_pairing 실패', ensureErr)

    const tokenReadable =
      typeof window !== 'undefined' &&
      (window.localStorage.getItem(DEVICE_TOKEN_STORAGE_KEY) ?? window.sessionStorage.getItem(DEVICE_TOKEN_STORAGE_KEY))

    if (!tokenReadable) {
      setErrorMessage('이 브라우저가 저장소를 막아 연결을 완료할 수 없어요. 일반 브라우저에서 열어 주세요.')
      setPairingPhase('error')
      return
    }
    ...
    window.setTimeout(() => window.location.assign(`${window.location.origin}/camera/broadcast?autostart=1`), 900)
  }
      ...
      <div className={styles.card}>
        ...
        {pairingPhase === 'success' ? (
          ...
          <>
            ...
            {pairingPhase === 'error' ? (
              <button className={styles.btnRetry} onClick={resetPairing}>다시 입력</button>
            ) : (
              <button className={styles.btnConnect} onClick={() => void submitPairingCode()} disabled={pairingPhase === 'pairing' || digits.join('').length < CODE_LENGTH}>
                {pairingPhase === 'pairing' && <span className={styles.spinner} aria-hidden />}
